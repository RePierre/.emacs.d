...
** Automatically export to pdf
   ...
   #+begin_src emacs-lisp
     ...
     (defun rp/toggle-export-to-pdf-async ()
       (interactive)
       (if (memq 'org-ref-export-to-pdf after-save-hook)
	   (progn
	     (remove-hook 'after-save-hook 'org-ref-export-to-pdf t)
	     (message "Disabled automatic export to pdf (via org-ref) for current buffer."))
	 (add-hook 'after-save-hook 'org-ref-export-to-pdf t t)
...
*** Uncategorized templates
    ...
    #+begin_src emacs-lisp
	 ...
	 ("f" "Monthly bill [Factură]"
	  entry
	  (file+olp+datetree "~/org/gtd/gtd.org" "Facturi")
	  "* TODO Factura %?\n DEADLINE: %^{Day}t\n %U" :tree-type month)
